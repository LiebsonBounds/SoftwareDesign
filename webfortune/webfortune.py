...
def scrape():
    ...
    #fortune is a random quote each time we run the program
    fortune = fortune_list[randint(0,(len(fortune_list)-1))]
    #print the fortune with (# comments)
        
    print fortune
    #calculate the length of the whole fortune
    fortune_length = len(fortune)
    #running a loop to search each character until we find a (, after the (, stop the rest
    catcher = 40
    for n in range(fortune_length):
        if fortune[n] == '(':
            catcher = n
    fortune_new = fortune[:catcher]
    print fortune_new
